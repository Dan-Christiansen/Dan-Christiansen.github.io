...
The command will calculate a diffusion constant from $$MSD$$ according to the Einstein relation
...
where $$N$$ is the total number of particles (atoms/molecules in the selection), $$\vec{v_i}$$ is a vector storing the three components of the velocity ($$v_x$$, $$v_y$$, and $$v_z$$) for the $$i$$-th particle.

$$\vec{v_i}(0)=\vec{v_i}(t=t_0)$$ and $$\vec{v_i}(t)=\vec{v_i}(t=t_0+n*\Delta*t)$$
